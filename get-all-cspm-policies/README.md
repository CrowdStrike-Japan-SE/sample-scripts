# About
このスクリプトは、CSPMのポリシーを取得してCSVに保存します。  
注意：カスタムポリシーは取得しません。


# 使い方
Falconコンソールで以下のAPIキーを作成。
```
...
実行。
```
curl -s https://raw.githubusercontent.com/CrowdStrike-Japan-SE/sample-scripts/refs/heads/main/get-all-cspm-policies/get-all-cspm-policies.py \
 | python3
```

all_cspm_policies.csv がカレントディレクトリに作成されます。
